...
#define FREQ_MAX_DEFAULT_VALUE4	S32_MAX
#define FREQ_MAX_DEFAULT_VALUE7	S32_MAX
#define FPS_60  60
#define FPS_90  90
#define FPS_120 120
#define FPS_UNKNOW -1
...
int cpufreq_table4[42]={2342400,2342400,2342400,2272200,2272200,2112000,2112000,2112000,1996800,1996800,1996800,1996800,
						1881600,1881600,1881600,1881600,1766400,1766400,1766400,1670400,1670400,1670400,1670400,1555200,
						1555200,1555200,1440000,1440000,1440000,1324800,1324800,1324800,1209600,1209600,1209600,1209600,
						1075200,1075200,1075200,1075200,960000,844800};

int cpufreq_table7[42]={2688000,2688000,2688000,2592000,2592000,2592000,2496000,2496000,2496000,2380800,2380800,2380800,
						2265600,2265600,2150400,2150400,2150400,2035200,2035200,2035200,2035200,2035200,2035200,2035200,
...
#else
	return dev_pm_qos_update_request(cdev->qos_req,cdev->freq_table[state].frequency);
#endif
}

void cpu_limits_level_set(unsigned int cpu, unsigned int max_freq)
{
		...
		if (cpufreq_dev->id == cpu) {
			for (level = 0; level <= cpufreq_dev->max_level; level++) {
				int target_freq = cpufreq_dev->freq_table[level].frequency;
	                        pr_debug("%s: target freq=%d \n", __func__, target_freq);
				if (max_freq >= target_freq) {
	                                pr_debug("%s: max_freq bigger than max_freq=%d \n", __func__, max_freq);
...
static int cpu_policy_init(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		ret = dev_pm_qos_add_request(get_cpu_device(cpu),
				cpufreq_dev->qos_req, DEV_PM_QOS_MAX_FREQUENCY,
				cpufreq_dev->freq_table[0].frequency);
#endif
		if (ret < 0) {
...
static void do_frame_limit_freq(void)
{
	...
	for_each_possible_cpu(i) {
		if(i==7){
			cpu_limits_level_set(i,cpufreq_table7[set_freq_level]);
		}else if(i>3){
			cpu_limits_level_set(i,cpufreq_table4[set_freq_level]);
		}
	}
}
...
static void do_maxfreq_release(void)
{
	...
	for_each_possible_cpu(i) {
		if(i==7){
			cpu_limits_level_set(i,FREQ_MAX_DEFAULT_VALUE7);
		}else if(i>3){
			cpu_limits_level_set(i,FREQ_MAX_DEFAULT_VALUE4);
		}
	}
}
...
static void do_maxfreq_release_work(struct work_struct *work)
{
	...
	do_maxfreq_release();
	pr_debug("do_maxfreq_release_work done\n");
}
...
caculate_fps:
	pr_debug("%s cur count time=%d", __func__, count_time);
...
void perfmgr_do_policy_V20(struct connected_buffer *cur)
{
	...
	pr_debug("%s cpufreq cur target fps=%d", __func__, cur->target_fps);
	pr_debug("%s cpufreq cur frame_count=%d", __func__, cur->frame_count);

	if((cur->target_fps==0)&&(cur->frame_count < FRAME_UNIT)){
		cur->last_frame_unit[cur->frame_count]=frame_usecs64;
		cur->frame_count++;
	        pr_debug("%s cpufreq cur frame cnt=%d", __func__, cur->frame_count);
	}else{
		...
		pr_debug("%s fixed target fps=%d", __func__, fixed_target_fps);
...
void perfmgr_do_policy_V10(struct connected_buffer *cur)
{
	...
	if(fixed_target_fps==-1){
		target_fps=calulate_fps(frame_usecs64,cur);
		pr_debug("%s cpufreq target fps=%d\n", __func__, target_fps);
		if(target_fps==-1){
...
void perfmgr_do_policy(struct connected_buffer *cur)
{
	static int count=0;
	++count;
	pr_err("skkk: PERFMGR_DO_POLICY %d", count);
    if(used_version_20)
